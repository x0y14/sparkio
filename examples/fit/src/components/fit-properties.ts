...
defineElement(
  {
    ...
    styles: css`
      ...
      .toggle-btn:hover { background-color: #3a3a5c; }
      .panel-header { @apply "flex justify-between items-center mb-3"; }
      .panel-title { @apply "text-xs font-semibold uppercase"; color: #aaa; }
      .section { @apply "mb-4"; }
      .section-title { @apply "mb-2"; color: #888; font-size: 11px; }
      label { @apply "text-xs block mb-1"; color: #ccc; }
      input[type='number'] {
        @apply "border rounded-md text-xs w-full box-border";
        background-color: #1a1a2e; border-color: #3a3a5c; color: #e0e0e0;
        padding: 4px 8px;
      }
      .row { @apply "flex gap-2"; }
      .row > * { @apply "flex-1"; }
      .kind-display {
        @apply "text-xs font-mono px-2 py-1 rounded";
        background-color: #1a1a2e; color: #8b8bff;
      }
    `,
  },
  () => {
      ...
      <div class="panel ${collapsed ? 'collapsed' : ''}">
        ...
        <div class="panel-content">
          <div class="section">
            ...
            <span class="kind-display">No selection</span>
          </div>
          <div class="section">
